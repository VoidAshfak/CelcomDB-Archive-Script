...
process_table() {
    ...
    retry_command "bcp \"$export_query\" queryout \"$bcp_remote\" -c -S \"$REMOTE_SERVER;TrustServerCertificate=yes\" -U \"$USER\" -P \"$SOURCE_PASS\"" \
...
for tbl in "${!TABLES[@]}"; do
    ...
    if ! process_table "$tbl" "$query" "$remote_path" "$local_path"; then
        export_failures+=("$tbl")
    fi
done
