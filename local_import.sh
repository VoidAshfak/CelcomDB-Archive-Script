...
for FILE in "$@"; do
    ...
    TABLENAME=$(echo "$BASENAME" | cut -d'_' -f1)

    # 4. Import
    log "Importing into CelcomDB_Archive_Shadhin_2025.dbo.${TABLENAME}_Temp"
    /opt/mssql-tools18/bin/bcp "CelcomDB_Archive_Shadhin_2025.dbo.${TABLENAME}_Temp" in "$FILE" \
        -c -b 100000 \
