...
export_table() {
    ...
    log "INFO" "$table_name exported successfully."
}



# ======================================
# PROCESS FUNCTION FOR IMPORT
# ======================================
import_table() {
    local table_name="$1"
    local export_query="$2"
    ...
    retry_command "ssh -p 9876 118.67.218.249 \"gunzip -f $bcp_local.gz\"" \
    ...
    log "INFO" "$table_name imported successfully."
}
...
for tbl in "${!TABLES[@]}"; do
    ...
    if ! export_table "$tbl" "$query" "$remote_path" "$local_path"; then
        export_failures+=("$tbl")
    fi
done

if [ ${#export_failures[@]} -gt 0 ]; then
    log "ERROR" "Export process completed with failures in: ${export_failures[*]}"
else
...
for tbl in "${!TABLES[@]}"; do
    IFS="|" read -r query remote_path local_path <<< "${TABLES[$tbl]}"
    if ! import_table "$tbl" "$query" "$remote_path" "$local_path"; then
        import_failures+=("$tbl")
    fi
done

if [ ${#import_failures[@]} -gt 0 ]; then
    log "ERROR" "Import process completed with failures in: ${import_failures[*]}"
else
    log "INFO" "All tables imported successfully."
fi
